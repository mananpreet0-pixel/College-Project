...
void transfer(int acc_no)
{
    ...
    FILE *log = fopen("lTransaction.txt","a");
    time_t t = time(NULL);
    fprintf(log,"Transferred %d from Acc %d to Acc %d | %s",amount,from,to,ctime(&t));
    fclose(log);
...
void transaction_detail(int acc_no)
{
    ...
    {
        char temp[20], sent[30], received[30], acc[20];
        sprintf(temp, "%d", acc_no);
        sprintf(sent, "from Acc %d", acc_no);
        sprintf(received, "to Acc %d", acc_no);
        sprintf(acc, "Acc %d", acc_no);

        if(strstr(line, "Deposited") && strstr(line, acc))
        {
            printf("Deposit: %s", line);
            found = 1;
        }
        else if(strstr(line, "Withdrawn") && strstr(line, acc))
        {
            printf("Withdraw: %s", line);
            found = 1;
        }
        else if(strstr(line, "Transferred") && strstr(line, sent))
        {
            printf("Sent: %s", line);
            found = 1;
        }
        else if(strstr(line, "Transferred") && strstr(line, received))
        {
